  ...
  implements OnTypeFormattingEditProvider
{
  ...
  lineMatchOpening(line: TextLine): boolean {
  ...
  indentationFor(line: TextLine, options: FormattingOptions) {
    return options.insertSpaces
      ? line.firstNonWhitespaceCharacterIndex / options.tabSize
      : line.firstNonWhitespaceCharacterIndex;
  }

  // This logic is mostly lifted from the original endwise extension so that we have the same behavior
  shouldAddEnd(
    document: TextDocument,
    lineNumber: number,
